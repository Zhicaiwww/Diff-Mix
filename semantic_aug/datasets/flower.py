...
class Flowers102Dataset(HugFewShotDataset):
    ...
    def __init__(self, *args, split: str = "train", seed: int = 0, 
        ...
        class_to_ids = {key: np.array_split(class_to_ids[key], [int(0.5*len(class_to_ids[key]))])[0 if split == "train" else 1] for key in self.class_names}
    ...
    def __init__(self, *args, split: str = "train", seed: int = 0, 
        ...
        super().__init__(
            *args, split=split, examples_per_class=examples_per_class,
            synthetic_probability=synthetic_probability, 
            return_onehot=return_onehot, soft_scaler=soft_scaler,
            synthetic_dir=synthetic_dir, image_size=image_size,crop_size=crop_size,
            **kwargs)
        ...
        if examples_per_class is not None and examples_per_class > 0:
            ...
            for key, items in label_to_indices.items():
                try:
                    ...
                    print(f"{key}: Sample {examples_per_class} larger than population {len(items)} or is negative, use random.choices instead")
...
class FlowersImbalanceDataset(Flowers102Dataset):
    ...
    def get_weighted_sampler(self):
        ...
        sampler = torch.utils.data.WeightedRandomSampler(samples_weight, len(self), replacement=True)
        return sampler
...
class FlowersImbalanceDatasetForT2I(FlowersDatasetForT2I):
    def __init__(self, *args, split: str = "train",
                 ...
                 weighted_alpha: int = 1,
                 **kwargs):
        super().__init__(*args, split=split, seed=seed, max_train_samples=max_train_samples, 
...
if __name__ == "__main__" :
    ds = FlowersDatasetForT2I(return_onehot=True)
    ds2 = Flowers102Dataset()

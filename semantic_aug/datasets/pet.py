...
class PetHugDataset(HugFewShotDataset):
    ...
    def __init__(self, *args, split: str = "train", seed: int = 0, 
        ...
        super(PetHugDataset, self).__init__(
        ...
        if split == 'train':
            dataset = load_dataset(HUB_LOCAL_DIR, split='train')
        else:
            dataset = load_dataset(HUB_LOCAL_DIR, split='test')
        ...
        self.class2label = {k.replace('/',' ').replace('_',' '): v for k, v in class2label.items()}
        self.label2class = {v: k.replace('/',' ').replace('_',' ') for k, v in class2label.items()} 
        self.class_names = [name.replace('/',' ').replace('_',' ') for name in dataset.features['label'].names]
...
class PetHugDatasetForT2I(torch.utils.data.Dataset):

...
from torchvision.models import resnet50, vit_b_16, swin_t,ViT_B_16_Weights, resnet18
from tqdm import tqdm
sys.path.append('/data/zhicai/code/Diff-Mix')
from utils import DATASET_NAME_MAPPING, checked_has_run
from downstream_tasks.utils import CutMix, mixup_data
from LabelSmoothing import LabelSmoothingLoss
from semantic_aug.datasets.cub import WaterBird
from datetime import datetime
# python train_hub.py -d cub -b 64 -g 6 -w 12 -s 2020 -a 2 -n train_hub_baseline_CE
...
def formate_note(args):
    ...
    note = f"{args.note}_base_{args.nepoch}_{args.optimizer}_{args.res_mode}_{args.model}_lr{args.lr}_{args.syn_type}_{args.seed}"
    if args.use_cutmix:
        note = note + '_cutmix'
    if args.use_mixup:
        note = note + '_mixup'
...
parser.add_argument('-d', '--dir', default='cub', help='dataset dir')
parser.add_argument('-m', '--model', default='resnet50', help='dataset dir')
parser.add_argument('-b', '--batch_size', default=32, help='batch_size')
parser.add_argument('-lr', default=None, type=float, help='learning rate')
parser.add_argument('--corrupt_prob', default=0, type=float, help='corruption probability')
parser.add_argument('--target_class_num', default=None, type=int, help='synthetic data dir')
parser.add_argument('--syn_p', default=0.1, type=float, help='synthetic data dir')
parser.add_argument('--weight_decay', default=5e-4, type=float, help='synthetic data dir')
parser.add_argument('--syn_type', default=None, type=str,nargs='+', help='synthetic data dir')
parser.add_argument('--clip_filtered_syn', default=False, action='store_true', help='synthetic data dir')
parser.add_argument('--use_cutmix', default=False, action='store_true', help='synthetic data dir')
parser.add_argument('--use_mixup', default=False, action='store_true', help='synthetic data dir')
parser.add_argument('--criterion', default='ls', help='dataset dir')
parser.add_argument(
...
exp_dir = 'outputs/result/{}/{}{}'.format(args.group_note, datasets_dir, args.note )  # the folder to save model
...
if args.syn_type is not None:
    from utils import parse_synthetic_dir
    synthetic_dir = parse_synthetic_dir(datasets_dir, args.syn_type)
else:
    synthetic_dir = None

return_onehot = True
gamma = args.gamma
soft_scaler = args.soft_scaler
synthetic_probability = args.syn_p
examples_per_class=args.examples_per_class
clip_filtered_syn=args.clip_filtered_syn
...
train_set = DATASET_NAME_MAPPING[datasets_dir](split='train',
                              image_size=re_size,
                              crop_size=crop_size, 
                              ...
                              clip_filtered_syn=clip_filtered_syn,
                              target_class_num=target_class_num,
                              corrupt_prob=corrupt_prob,
                              )

test_set = WaterBird(split=2, image_size=256, crop_size=224, 
                     return_onehot = return_onehot)
# test_set = DATASET_NAME_MAPPING[datasets_dir](split='val',
#                              image_size=re_size,
#                              crop_size=crop_size,
#                               return_onehot=return_onehot
#                              )
...
if args.use_cutmix:
    train_set = CutMix(train_set, num_class=train_set.num_classes, prob=args.mixup_probability)
train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, collate_fn=collate_fn, num_workers=num_workers)
eval_loader = DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=num_workers)
...
if args.model == 'resnet18':
    ...
    net = swin_t(pretrained=True,num_classes=nb_class)
...
if args.finetune_strategy is not None and args.model == 'resnet50':
    from base.utils import freeze_model
...
if args.optimizer=='sgd':
    optimizer = torch.optim.SGD(
        net.parameters(), lr=lr_begin, momentum=0.9, weight_decay=args.weight_decay,
    )
elif args.optimizer=='adamw':
    optimizer = torch.optim.AdamW(
        net.parameters(), lr=lr_begin,
    )
else:
    raise ValueError('optimizer not supported')
...
shutil.copyfile('downstream_tasks/cls/train_hub_waterbird.py', exp_dir + '/train_hub_waterbird.py')
# save args to yaml
    ...
    if epoch % 4 == 0:
        ...
        for _, (batch) in enumerate(tqdm(eval_loader, ncols=80)):
            ...
            for i, group in enumerate(groups):
                ...
                group_stats[group]['correct'] += int(predicted[i].eq(targets[i].data).cpu())
        ...
        for group, stats in group_stats.items():
            group_acc = 100.0 * stats['correct'] / stats['total'] if stats['total'] > 0 else 0.0
            print('Group {}: {:.3f}% ({}/{})'.format(group, group_acc, stats['correct'], stats['total']))
        ...
        with open(os.path.join(exp_dir, 'train_log.csv'), 'a+') as file:
            file.write(
                '{}, {:.4f}, {:.4f}, {:.3f}%, {:.3f}%'.format(
                    epoch, lr_now, train_loss, eval_acc, max_eval_acc
                )
            )
            for group, stats in group_stats.items():
                group_acc = 100.0 * stats['correct'] / stats['total'] if stats['total'] > 0 else 0.0
                file.write('{}, {:.3f}%'.format(group, group_acc))
            file.write('\n')

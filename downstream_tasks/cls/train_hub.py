...
def formate_note(args):
    ...
    note = f"{args.note}_base_{args.nepoch}_{args.optimizer}_{args.res_mode}_{args.model}_lr{args.lr}_{args.syndata_key}_{args.seed}"
    if args.use_cutmix:
        note = note + '_cutmix'
    if args.use_mixup:
        note = note + '_mixup'
...
parser.add_argument('--syndata_key', default=None, type=str,nargs='+', help='synthetic data dir')
...
if args.syndata_key is not None:
    from utils import parse_synthetic_dir
    synthetic_dir = parse_synthetic_dir(datasets_dir, args.syndata_key)
else:
    synthetic_dir = None
...
shutil.copyfile('downstream_tasks/cls/train_hub.py', exp_dir + '/train_hub.py')
# save args to yaml
